...
import MobileNotesContext from "../NotesContextMobile/MobileNotesContext";
...
function NotesMobilePage() {
  ...
  return (
    <div className="mobiles__notes__page">
      <div className="mobile__notes__context__title">
        <img src={back} alt="back" onClick={goBack} />
        <div
          className="mobile__notes__context__title__color"
          style={{ backgroundColor: bgColor }}
        >
          {initials}
        </div>
        <div className="mobile__notes__context__title__text">
          {selectedTitle}
        </div>
      </div>
        ...
        {notes.length === 0 ? (
          ...
          <div>
            {notes.map((note, index) => (
              <MobileNotesContext key={index} note={note} />
            ))}
          </div>
        )}
      </div>
